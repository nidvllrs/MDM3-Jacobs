...
def read_ids_network(csv):
    ...
    Output:
        - network_sett: set of stations ids and station names including 'splits'
    """
    i=1
...
def draw_graph(edgelist, passengers, origin, destination):
    ...
    # nodes =  ['Ashburys', 'Split 10', 'Manchester Piccadilly', 
    #           'Split 25', 'Ardwick', 'Split 12', 'Split 15', 
    #           'Mauldeth Road', 'Burnage', 'East Didsbury', 
    #           'Gatley', 'Stockport', 'Split 35', 'Davenport', 
    #           'Woodsmoor', 'Split 34', 'Split 32', 'Navigation Road',
    #           'Altrincham', 'Levenshulme', 'Heaton Chapel']
    # splits = [i for i in nodes if 'Split' in i]
    # stations = [i for i in nodes if i not in splits]
    ...
    nx.draw_networkx_nodes(G, pos=pos,ax=ax,  nodelist = stations, node_color="b", node_size = 1000, alpha=0.2, label='Stations')
    nx.draw_networkx_nodes(G, pos=pos,ax=ax, nodelist = splits, node_color="r", node_size = 600, alpha=0.2, label='Splits')
    nx.draw_networkx_labels(G, pos=pos,ax=ax, font_size= 30)
    ...
    # nx.draw_networkx_edge_labels(G, pos=pos, ax=ax, edge_labels=edge_labels, rotate=False,  font_size= 40)
...
def disruptions (G, nodes, passengers, origin, destination):
    ...
    # Remove path between origin and destination (+ return) 
    ...
    # Condition for when edges might not have an alternative route:
    if nx.has_path(G2, source = origin, target = destination)== True:
        ...
        # Alternative psth only to include stations and not splits
    ...
    # Resetting graph with new edge weights / new nodes (for zoomed in version)
...
passengers, edgelist = read_csv_passangers('edge_flows_1.csv', network_sett)
# print('passangers:\n', passangers)
...
# print('p_affected_set',p_affected_set)
# print('ind_p_affected_set',ind_p_affected_set)
# ========================================================================
# =============================================================================
